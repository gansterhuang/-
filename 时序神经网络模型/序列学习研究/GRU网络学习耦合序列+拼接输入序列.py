...
plt.subplot(312)
plt.plot(range(np.shape(x2)[0]),x2)
plt.subplot(313)
plt.plot(range(1000),x3)
plt.show()
...
data=np.zeros((data_number,9),dtype=float)#定义具有（500，9）格式的初始数组
for i in range(data_number):
    data[i]=np.array([x1[i],x2[i],x1[i+1],x2[i+1],x1[i+2],x2[i+2],x1[i+3],x2[i+3],x3[i+4]])#输入X1,X2序列，预测X3
            #这里数组里前4个数为输入，后一个为输出
...
#训练集输入
data_x=np.zeros((data_number,4,2),dtype=float)#定义如（500,4）的格式
for i in range(data_number):
    data_x[i]=[[data_random[i][0],data_random[i][1]],
               [data_random[i][2],data_random[i][3]],
               [data_random[i][4],data_random[i][5]],
               [data_random[i][6],data_random[i][7]]
               ]
...
weights = {
    ...
    #这里的out_hidden加以改进
    'out_hidden': tf.Variable(tf.random_normal([rnn_unit+time_step*input_size, out_hidden_size])),
...
def lstm(batch):
    ...
    #构建后续用于拼接的序列X
    input_for_concat=tf.reshape(x,[batch,-1])
    ...
    #进行拼接
    concat_output=tf.concat([output,input_for_concat],1)
    ...
    out_hidden=tf.nn.tanh(tf.matmul(concat_output,w_out_hidden)+b_out_hidden)
...
#测试集输入，
test_number=950
test_data=np.zeros((test_number,4,2),dtype=float)#定义具有（500，4,2）格式的初始数组
for i in range(test_number):
    test_data[i]=np.array([[x1[i],x2[i]],
                           [x1[i+1],x2[i+1]],
                           [x1[i+2],x2[i+2]],
                           [x1[i+3],x2[i+3]]])
            #这里数组里前4个数为输入，后一个为输出
...
rmse = 0.0
for i in range(test_number):
    rmse=rmse+(pre_list_np[i]-x3[i+4])**2
...
#绘制图像
plt.plot(range(np.shape(x3)[0]), x3)
plt.plot(range(np.shape(x3)[0])[4:4+test_number], pre_list_np, color='r')
